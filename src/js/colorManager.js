...
function generateCssVariableName(category, colorName) {
  ...
  Object.keys(colorData).forEach((category) => {
    if (Array.isArray(colorData[category]) && colorData[category].length > 0) {
      ...
      if (firstColor && firstColor.hex) {
        ...
        if (category === "primary") {
          ...
          if (
            ...
            const lightHex = lightenColor(firstColor.hex, 20);
            document.documentElement.style.setProperty(
              UI_CSS_MAPPINGS.primaryLight.cssVar,
              lightHex,
            );
          ...
          if (
            ...
            const darkHex = darkenColor(firstColor.hex, 20);
            document.documentElement.style.setProperty(
              UI_CSS_MAPPINGS.primaryDark.cssVar,
              darkHex,
            );
          ...
          colorData[category].forEach((color, index) => {
            if (color && color.hex) {
              ...
              const nameBasedCssVar = generateCssVariableName(
                category,
                color.name,
              );
              document.documentElement.style.setProperty(
      ...
      colorData[category].forEach((color, index) => {
        ...
        const nameBasedCssVar = generateCssVariableName(category, color.name);
...
export function initColorManager(colorData) {
  ...
  return {
    updateColor: (category, colorName, newHexValue) => {
      const result = updateColor(colorData, category, colorName, newHexValue);
      if (result) {
